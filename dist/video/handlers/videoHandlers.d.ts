import { Request, Response } from 'express';
export declare const getVideoByIdHandler: (req: Request, res: Response) => void;
export declare const createVideoHandler: (req: Request, res: Response) => void;
export declare const getVideos: (req: Request, res: Response) => void;
export declare const updateVideosHandler: (req: Request, res: Response) => Response<any, Record<string, any>>;
export declare const deleteVideoByIdHandler: (req: Request, res: Response) => Response<any, Record<string, any>>;
//# sourceMappingURL=videoHandlers.d.ts.map